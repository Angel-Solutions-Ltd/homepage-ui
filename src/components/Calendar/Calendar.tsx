...
export const Calendar = ({}: P) => {
    ...
    const renderMonthsAndDays = useMemo(() => COPY.calendar.months.map((month, monthIndex) => {
        ...
        return (
            <div key={month} className={`month ${isActiveMonth ? 'active-month' : ''}`}>
                ...
                <div className="weekdays">
                    {COPY.calendar.days.map((day, key) => (
    ...
    const changeMonth = (delta: number) => setCurrentMonth(prev => Math.min(11, Math.max(0, prev + delta)));
    ...
    return (
        <div className="calendar-outer">
            ...
            {currentInput !== null && (
                <div className="day-input-holder">
                    <div>{COPY.calendar.addActivity} {formatDate(currentInput)}</div>
                    <input className="day-input" type="text" />
                    <button>{COPY.calendar.save}</button>
                </div>
            )}
